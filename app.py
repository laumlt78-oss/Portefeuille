...
# --- 3. MOTEUR DE GRAPHIQUES (VOTRE VERSION PRÉFÉRÉE) ---
...
if st.session_state.mon_portefeuille:
    for i, act in enumerate(st.session_state.mon_portefeuille):
        try:
            ...
            if not h.empty:
                # Gestion MultiIndex éventuelle ici aussi
...
with tab_p:
    ...
    for item in donnees_pos:
        ...
        with st.expander(header):
            ...
            if st.button("Supprimer l'action", key=f"del_{idx}"):
    ...
    if tickers_list:
        ...
        with h1:
            ...
            if isinstance(data, pd.DataFrame):
                ...
                tracer_courbe(pd.DataFrame({'Close': val_port}), "Valeur totale du portefeuille (7j)")

# --- 5. SIDEBAR (LE BANDEAU DE GAUCHE) ---
with st.sidebar:
    st.header("⚙️ Gestion")
        ...
        if st.form_submit_button("Ajouter au portefeuille"):
            if f_n and f_t:
                st.session_state.mon_portefeuille.append({
                    "Nom": f_n, "Ticker": f_t.upper(), "PRU": f_p, "Qté": f_q, "Date_Achat": str(f_d)
                })
                sauvegarder_vers_github(st.session_state.mon_portefeuille)
                st.rerun()
